...
phoneInput.addEventListener('input',async function(){
  ...
  function checkNumberAndCallApi(phone){
    console.log("PHONE",phone);
    const cleaned = phone.replace(/\D/g,''); // removing none digit chars
    console.log("PHONE-clean",cleaned);
   
    //check for 9 digits 
    //if(cleaned.length === 9 && !/^0|^256/.test(cleaned)){
    let trimmed = cleaned;
    ...
    if(trimmed.length === 9){
     statusIcon.innerHTML = '<span class="loader"></span>';
      ...
      console.log("--NUM.length",trimmed.length)
      fetch(`https://pay.yeshualifeug.com/userinfo`,{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ phonenumber: '256' + trimmed })

      })
      .then(res => res.json())
      .then(data=>{
        console.log("retturned..");
        console.log(data);
        if(data){
         statusIcon.textContent = '✅';
         statusIconName.textContent = '✅';
        const name = data['family_name'] + " " + data['given_name'];
        document.getElementById('donorName').value = name;
        paydonation.disabled = false;
      ...
      .catch(err => {
        ...
        statusIconName.textContent = '⚠️';
        console.error('API error:', err)
      });
    }else {
      statusIcon.textContent = ''; // reset if not a valid number yet
    }
